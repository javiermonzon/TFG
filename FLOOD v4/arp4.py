...
# Si no pasamos nada como parámetro -> arp_flood.log
...
if len(sys.argv) > 1:
    ...
    log_filename = f"arp_flood_{now.day:02d}_{now.month:02d}__{now.hour:02d}_{now.minute:02d}.log"
        ...
        if i < (len(speed_list) - 1):
            ...
            while(abs(speed_next - speed) > max_size_transition):
                if(speed_next > speed):
                    ...
                    flood_arp(src_mac, src_ip, dst_ip, threads, speed, time_transition_random) # 5 segundo para cada iteración de rampa de subida/bajada  
                else: 
                    speed -= max_size_transition
